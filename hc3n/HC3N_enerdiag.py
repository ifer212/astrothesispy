...
def HC3N_energydiag(save_fig_path):
    # HC3N CDMS data 
    HC3N_dir = '/anaconda2/envs/ferpy/lib/python3.7/site-packages/ferpy/molecules/HC3N/CDMS/HC3N/'
    
    
    # Out dir Figure
    out_fig1_dir = save_fig_path#HC3N_dir+'/Figure/'
    ...
    freq_hz = utiles.trans_freq(ene_K)
    ...
    ax.set_ylabel('Energy (K)', fontsize=labelsize)#, fontsize=labelsize, fontname=labelfont, fontweight='bold')
    ...
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3.eps', bbox_inches='tight', transparent=True, dpi=400)
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3.pdf', bbox_inches='tight', transparent=True, dpi=400)
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3.png', bbox_inches='tight', transparent=True, dpi=400)
    plt.close()
    ...
    for i, row in df_dict['v7=1']['df'].iterrows():
        ...
        if row['jlow'] in ['16', '24', '26', '39']:
            freq_E = utiles.trans_energy(row['freq_MHz']/1000)
            if (row['llow'] == 1.0):
                strplt = row["jlow"]
    ...
    #            if row['jlow'] in ['39']:
    #                print(row['Elo_K'])
    #                print(df_dict['v7=1']['df'].iloc[i+1]['Elo_K'])
    #                ax2.annotate("", xy=(2.25, row['Elo_K']),
    #                    xytext=(2.25, row['Elo_K']+freq_E),
    #                    arrowprops=dict(arrowstyle="<->, head_length=0.2, head_width=0.1",
    #                                #connectionstyle="arc3,rad=-4",
    ...
    freq_E = utiles.trans_energy(df39_p1_38_m1['freq_MHz']/1000)
    ...
    freq_E = utiles.trans_energy(df24_p1_23_m1['freq_MHz']/1000)
    ...
    freq_E = utiles.trans_energy(df26_p1_25_m1['freq_MHz']/1000)
    ...
    freq_E = utiles.trans_energy(df16_p1_15_m1['freq_MHz']/1000)
    ...
    #ax2.text(2.75, 330, r'$J$',ha='center', va='center', backgroundcolor='none', fontsize=8 , color='k', fontweight='bold')
    ...
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3all.eps', bbox_inches='tight', transparent=True, dpi=400)
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3all.pdf', bbox_inches='tight', transparent=True, dpi=400)
    fig.savefig(out_fig1_dir+'/hc3n_Ediag_K_v3all.png', bbox_inches='tight', transparent=True, dpi=400)
    plt.close()
