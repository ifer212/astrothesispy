...
fit_logn = False
plot_models = False
plot_models_v2 = True # SHC13
SHC3_plot = False
mod_difs = False

model_summary = False

models_absorption = False
comp_LTEvsNLTE = False
comp_LTEvsNLTE_v15 = False
...
# Ed models
ed_model_path = userpath+'/Documents/Ed/transf/program/models/'
my_model_path = userpath+'/Documents/Ed/transf/program/models/mymods/'
my_model_figs = userpath+'/Documents/Ed/transf/program/SHC13/figures_mymods/'
fort_paths = userpath+'/Documents/Ed/transf/program/SHC13/'
dustmod_radpc = 17 # All dustmodels are ran with r=17pc
ed_fig_save_path = '/Users/frico/Documents/Ed/transf/program/models/model_madcube_comp/'
final_results_path = '/Users/frico/Documents/data/NGC253_HR/Results_v2/'
# Observed fluxes 
hb_path = path+'/'+SHC+'/EdFlux/'+SHC+'_flujos_hc3n_def_v5.xlsx'
hb_df = pd.read_excel(hb_path, header=0)
new_hb_path = path+'/SHC_13/EdFlux/'+SHC+'_flujos_hc3n_python.xlsx'
...
# Results paths
results_path = userpath+'/Documents/data/NGC253_HR/Results_v2/'
if not os.path.exists(results_path):
    os.makedirs(results_path)
fig_path = results_path+'Figures/'
if not os.path.exists(fig_path):
    os.makedirs(fig_path)
figrt_path = results_path+'Figures_radtransf/'
...
if load_observed_results:
    obs_df = pd.read_csv('/Users/frico/Documents/data/NGC253_HR/Results_v2/SHC_13_SLIM_Tex_and_logN_profiles.csv', sep=';')
...
if spec_index:
    ...
    beta = 1.5
    cubo_111_path = '/Users/frico/Documents/data/NGC253_HR/Continuums//MAD_CUB_MOD_111GHz_spw31_continuum_mfs.I.manual.image.pbcor.fits'
    cubo_111 = utiles.Cube_Handler('111', cubo_111_path)
    rms_111 = 4.303E-5
    ...
    Leroy36df_new['S219_SM_nondust_int_Q0'] = utiles.ion_prod_rate(f219, Leroy36df_new['S219_SM_nondust_int_L_erg/s'], Te)
    Leroy36df_new['S219_SM_nondust_int_MZAMS_Msun'] = Leroy36df_new['S219_SM_nondust_int_Q0']/4e46
    Leroy36df_new['S219_SM_nondust_int_Q0_J'] = utiles.ion_prod_rate_j(f219, Leroy36df_new['S219_SM_nondust_int_Jy'], Te, D_Mpc*1000)
    ...
    Leroy36df_new['S111_int_Q0_J'] = utiles.ion_prod_rate_j(f111, Leroy36df_new['111GHz_Int_fit_Jy'], Te, D_Mpc*1000)
...
if plot_models:
    Rcrit = 0.85
    sbagnmodsbestv2= { 'model2': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt5_b9','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
    ...
    modelos = sbagnmodsbestv2
    # Plot model set
    
    #utiles_nLTEmodel.plot_models_and_inp(line_column, modelos, new_hb_df, cont_df, my_model_path, figmod_path, figrt_path, fort_paths)
    
    # Final figures
    #utiles_nLTEmodel.plot_models_and_inp_finalfig(Rcrit, line_column, modelos, new_hb_df, cont_df, my_model_path, finalfigmod_path, finalfigmod_path, fort_paths)
    ...
    for i,modelo in enumerate(modelos):
        ...
        #utiles_nLTEmodel.plot_models_and_inp(Rcrit, line_column, modsing, new_hb_df, cont_df, my_model_path, figmod_path, figrt_path, fort_paths)
...
if SHC3_plot:
    finalfigmod_path = my_model_figs+'Models_final_SHC3/'
    if not os.path.exists(finalfigmod_path):
        os.makedirs(finalfigmod_path)
    Rcrit = 0.35
    rad_pc = 0.50
    ...
    modelos = bestmodsv2
    utiles_nLTEmodel.plot_models_and_inp_finalpaperfig(Rcrit, line_column, modelos, new_hb_df, cont_df, my_model_path, finalfigmod_path, finalfigmod_path, fort_paths, source_rad=rad_pc)


if models_absorption:
    Rcrit = 0.85
    ...
    #utiles_nLTEmodel.read_model_input(bestmod['model2'], my_model_path, Rcrit)
    ...
    colors_r = colors#[::-1]
    for m,mod in enumerate(sort_dict):
        #modelo = mod.split('/')[-1].split('.inp')[0]
        #mname = modelo.split('_')[-1]
...
if comp_LTEvsNLTE:
    ...
    finalfigmod_path = my_model_figs+'Modelscomp_final/'
    if not os.path.exists(finalfigmod_path):
        os.makedirs(finalfigmod_path)
    ...
    #24 26 111 117 336 340
    line_column = {'plot_conts': [],
               'v=0_24_23_SM': [15, r'$v=0$  26 - 25', [-10, 150], 1], 
               'v=0_26_25_SM': [15, r'$v=0$  26 - 25', [-10, 150], 2], 
               'v7=1_24_1_23_-1_SM': [15, r'$v_{7}=1$  24,1 - 23,-1', [-10, 150], 3],
               'v7=1_26_1_25_-1_SM': [15, r'$v_{7}=1$  26,1 - 25,-1', [-10, 150], 4],
               'v6=1_24_-1_23_1_SM': [15, r'$v_{6}=1$  24,-1 - 23,1', [-10, 74], 5],
               'v6=1_26_-1_25_1_SM': [15, r'$v_{6}=1$  26,-1 - 25,1', [-10, 74], 6],
               }
...
if comp_LTEvsNLTE_v15:
    
    finalfigmod_path = my_model_figs+'Modelscomp_final_v15/'
    if not os.path.exists(finalfigmod_path):
        os.makedirs(finalfigmod_path)
    ...
    Rcrit = 0.85
    compmods1= { 'LTE v5': ['m28_cLTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt5_b9','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, redpink , [1, 1.0, 1.9], '-', True],
                'LTE v15': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt15_b9','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, azure, [1, 1.0, 1.9], '-', True],
                'LTE v15c3': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt15_b9_c3','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, azure, [1, 1.0, 1.9], '--', True],
                'LTE v15c4': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt15_b9_c4','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, 'b', [1, 1.0, 1.9], '--', True],
                'LTE v15c5': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt15_b9_c5','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, redpink, [1, 1.0, 1.9], '--', True],
                'LTE v10a3': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt10_b9_a3','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  'lime', [1, 1.0, 1.9], '-', True],
                'LTE v10v20c1': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt10_b9_c1','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  green, [1, 1.0, 1.9], '-', True],
                'LTE v10v20c2': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt10_b9_c2','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  green, [1, 1.0, 1.9], '-.', True],
                'LTE v7a1': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt7_b9_a1','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  violet, [1, 1.0, 1.9], '-', True],         
                'LTE v7a2': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt7_b9_a2','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  violet, [1, 1.0, 1.9], '--', True],        
                'LTE v7c2': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt7_b9_c2','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  'k', [1, 1.0, 1.9], '-', True],   
                'LTE v7c3': ['m28_LTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt7_b9_c4','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5,  'k', [1, 1.0, 1.9], '--', True],     
                                 

               }
    
    compmods2= { 'LTE v5': ['m28_cLTHC3Nsbsig1.3E+07cd1.0E+25q1.5nsh30rad1.5vt5_b9','dustsblum1.2E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, redpink , [1, 1.0, 1.9], '-', True],
                'LTE HL v15': ['m24_LTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7','dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, azure, [1, 1.0, 1.9], '-', True],
                               
                'NLTE HL v15': ['m24_NLTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7', 'dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, green, [1, 1.0, 1.9], '--', False],
                'NLTE HL v10v15': ['m24_NLTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7_c3', 'dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, dgreen, [1, 1.0, 1.9], '--', False],
                'NLTE HL v10v15v20': ['m24_NLTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7_c5', 'dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, violet, [1, 1.0, 1.9], '--', False],
                'NLTE HL v10v15v20c': ['m24_NLTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7_c6', 'dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, dviolet, [1, 1.0, 1.9], '--', False],
                'NLTE HL v10v15v20d': ['m24_NLTHC3Nsbsig5.5E+07cd1.0E+25q1.5nsh30rad1.5vt15_a7_c7', 'dustsblum5.0E+10cd1.0E+25exp1.5nsh1003rad17',
                            1.5, 'k', [1, 1.0, 1.9], '--', False],
             }
...
if model_summary: 
    ...
    if redo_lum:
        ...
        for mod in modelos:
            ...
            mod_dict = utiles_nLTEmodel.model_summary(modelo, dustmod, my_model_path, distance_pc, Rcrit)
            modsum_df = modsum_df.append(mod_dict, ignore_index=True)
        modsum_df.to_excel(final_results_path+SHC+'_finalmods_modsummary_Rcrit'+str(Rcrit).replace('.','p')+'.xlsx')  
        
        
        
        
    modsum_df_rcrit0p5 = pd.read_excel(final_results_path+SHC+'_finalmods_modsummary_Rcrit0p5.xlsx', header=0)
    ...
    modsum_df = pd.read_excel(final_results_path+SHC+'_finalmods_modsummary_Rcrit'+str(Rcrit).replace('.','p')+'.xlsx', header=0)
    ...
    modsum_df[['half_rad_pc', 'half_rad_pcv2', 'half_rad_pc_corr']]
    
    modsum_df['SigmaH2_cm2']
    ...
    bgn_df = pd.read_excel(final_results_path+'BGN_properties.xlsx', header=0)
    hc_df  = pd.read_excel(final_results_path+'HCs_properties.xlsx', header=0)
    
    pfalzner_df = pd.read_excel('/Users/frico/Documents/data/NGC253_HR/Results_v2/SSC_tables/pfalzner2009.xlsx', header=1)
    ...
    lada_df['Lum_Lsun'] = lada_df['Mass_Msun']*light_to_mass#10**(0.4*(MK_sun-lada_df['K_limit']))
    ...
    portout_df['Lum_Lsun'] = 10**(0.4*(Mv_sun-portout_df['Mv_mag']))#light_to_mass*10**portout_df['log_Mphot']#
    ...
    u_conversion.lin_size(450*1e-6, 4/2)
    ...
    for i, row in bgn_df.iterrows():
        #if np.isnan(row['SigmaIR2_Lsun_pc2']):
        #    bgn_df.loc[i, 'SigmaIR2_Lsun_pc2'] = 
    ...
    #SHC_compfig_helper.plot_LIR_comp(modsum_df, hc_df, rolffs_df, bgn_df, final_results_path,
    ...
    SHC_compfig_helper.plot_LIR_comp_ALL(modsum_df, hc_df, rolffs_df, bgn_df, final_results_path
                                         , pfalzner_df, lada_df, portout_df,portin_df, Lmod_err=0.5, only_HC = True)
    
    SHC_compfig_helper.plot_LIR_comp_ALL_big(modsum_df, hc_df, rolffs_df, bgn_df, final_results_path
                                         , pfalzner_df, lada_df, portout_df,portin_df, Lmod_err=0.5, only_HC = True)
    ...
    if ind_plots:
        ...
        fig.savefig(final_results_path+'LIR_R_LIRMgasSigma.pdf', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
        ...
        fig.savefig(final_results_path+'LIRMgas_Sigma.pdf', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
        ...
        fig.savefig(final_results_path+'LIRMgas_Lir.pdf', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
