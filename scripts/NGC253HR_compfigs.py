...
def plot_LIR_comp(modsum_df, hc_df, rolffs_df, bgn_df, final_results_path, pfalzner_df, lada_df, portout_df,portin_df, Lmod_err=0.5, only_HC = True):
    ms = 12
    figsize = 8
    naxis = 3
    maxis = 1
    ...
    ticksize = 18
    fontsize = 14
    fig = plt.figure(figsize=(figsize*2.95, figsize*0.85))
    ...
    for i, row in rolffs_df.iterrows():
        ...
        axis[1].annotate(name, xy=(x_val,y_val), xytext=(x_val, y_val),
    ...
    axis[2].set_xlabel(r'$\log{L_{\text{IR}}}$ (L$_{\odot}$)', fontsize=labelsize)
    axis[2].set_ylabel(r'$\log{\Sigma_{\text{IR}}}$ (L$_{\odot}$/pc$^{2}$)', fontsize=labelsize)
    ...
    fig.savefig(final_results_path+'LIR_comp_v4.pdf', bbox_inches='tight', transparent=True, dpi=400)
    plt.close()
       
def plot_LIR_comp_ALL(modsum_df, hc_df, rolffs_df, bgn_df, final_results_path, pfalzner_df, lada_df, portout_df,portin_df, Lmod_err=0.5, only_HC = True):
    ms = 12
    figsize = 8
    naxis = 2
    maxis = 2
    ...
    gs = gridspec.GridSpec(maxis, naxis)#, width_ratios=[1,1,1,0.1], height_ratios=[1])    
    gs.update(wspace = 0.18, hspace=0.15, top=0.95, bottom = 0.05, left=0.05, right=0.80)
    ...
    for i, row in bgn_df.iterrows():
        ...
        if row['Source'] == 'Arp220E':
            ...
            ym = 0.03
        axis[1].annotate(row['Source'], xy=(x_vals[ind_text],y_vals[ind_text]), xytext=(x_vals[ind_text]+xm,y_vals[ind_text]+ym),
                 ha=halign, va=valign, color = 'k') #arrowprops={'arrowstyle': '-', 'color': 'k'},
    if panel_2_MH2_R2:
