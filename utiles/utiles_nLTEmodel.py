...
def plot_models_and_inp_finalpaperfig(convolve, Rcrit, line_column, modelos, hb_df, cont_df,
    ...
    if cont_Tex_N_X_fig_BIG:
        ...
        gs1 = gridspec.GridSpec(maxis, naxis)
        gs1.update(wspace = 0.15, hspace=0.0, top=0.95, bottom = 0.05, left=0.05, right=0.80)
        gs1.update(wspace = 0.15, hspace=0.0, top=0.95, bottom = 0.05)
        axes=[]
        mykeys_conts = ['plot_conts', 'plot_T', 'plot_col', 'plot_x']
        for l,line in enumerate(mykeys_conts):
            axes.append(fig.add_subplot(gs1[l]))
                ...
                if line == 'plot_conts':
                    ...
                    if writename:
                        ...
                        axes[l].text(0.95, ytext, modstr+':  '+Lstr +' \; '+qstr,
                                    ...
                                    transform=axes[l].transAxes)
                    ytext = ytext -0.045
                    axes[l].set_ylabel(r'T$_{\text{dust}}$ (K)', fontsize = labelsize)
                    for i,row in obs_df.iterrows():
                        if i <= 2 or i>= 10:
                            axes[l].errorbar(row['dist_ring_pc'], row[tstring], 
                                                        uplims = True,
                    ...
                    axes[l].set_ylabel(r'$n_{\text{H}_{2}}$ (cm$^{-3}$)', fontsize = labelsize)
                    axes[l].plot(rpc_profile[1:], nh2_profile[1:], color=mod_color, zorder=mzord)
                    axes[l].set_yscale('log')
                elif line == 'plot_col':
                    for i, row in obs_df.iterrows():
        ...
        if plot_opacity:
            # Plotting opacity
            gs2 = gridspec.GridSpec(maxis, naxis)    
            gs2.update(wspace = 0.23, hspace=0.23, top=0.95, bottom = 0.05, left=0.05, right=0.80)
            axtau = fig.add_subplot(gs2[-1])
            for m, mod in enumerate(modelos):
                mod_name = modelos[mod][0]
        ...
        if len(modelos) == 1:
            for m, mod in enumerate(modelos):
                save_name = modelo = modelos[mod][0]
            fig.savefig(f'{figmod_path}{fig_name[0]}NGC253_{save_name}_conts{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        else:
            fig.savefig(f'{figrt_path}{fig_name[0]}NGC253_{save_name}_conts{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
    ...
    if line_profiles:
        ...
        if len(modelos) == 1:
            for m, mod in enumerate(modelos):
                save_name = modelo = modelos[mod][0]
                fig.savefig(f'{figmod_path}{fig_name[1]}NGC253_{save_name}_lines_{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        else:
            fig.savefig(f'{figrt_path}{fig_name[1]}NGC253_{save_name}_lines_{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
    ...
    if line_ratios_BIG:
        ...
        if len(modelos) == 1:
            for m, mod in enumerate(modelos):
                save_name = modelo = modelos[mod][0]
                fig.savefig(f'{figmod_path}{fig_name[2]}NGC253_{save_name}_ratios{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        else:
            fig.savefig(f'{figrt_path}{fig_name[2]}NGC253_{save_name}_ratios{convstr}{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
    ...
    if line_ratios_BIG_onecol:
        ...
        fig = plt.figure(figsize=(figsize*naxis/maxis*1.15, figsize*0.85))
        gs1 = gridspec.GridSpec(maxis, naxis)
        gs1.update(wspace = 0.13, hspace=0.0, top=0.95, bottom = 0.05)
        xlabel_ind = 0
        mykeys_flux = ['v71_v61_2423', 'v6_v6v7']
        xtextpos = 0.18
        ytextpos = 0.95
        ...
        if len(modelos) == 1:
            for m, mod in enumerate(modelos):
                save_name = modelo = modelos[mod][0]
                fig.savefig(f'{figmod_path}{fig_name[2]}NGC253_{save_name}_ratios{convstr}_onecol{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        else:
            fig.savefig(f'{figrt_path}{fig_name[2]}NGC253_{save_name}_ratios{convstr}_onecol{fig_format}', bbox_inches='tight', transparent=True, dpi=400)
        plt.close()
